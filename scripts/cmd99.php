...
// 2020-07-24 : 클래스 스케쥴링 시 참조할 DB 선택
//$schedule_tb_chk = trim(read_conf("/usr/lib/qosd/conf/class_schedule_set.conf", "class_schedule")); 
$class_table_chk = trim(read_conf("/usr/lib/qosd/conf/class_schedule_set.conf", "class_table_chk")); 
if ( $class_table_chk == "2" ) {
	$i_tb_name = "tblClass_int_test" ;
	$e_tb_name = "tblClass_ext_test" ;
} else {
	$i_tb_name = "tblClass_int" ;
	$e_tb_name = "tblClass_ext" ;
}
...
$q_result = mysqliQuery($q_query, $dbconn);
...
if ( $dir == "i" ) {
	...
	$sql = "select cid, cname, depth from $i_tb_name where hid_qid=$qname_id  and is_view='1' and is_drop != '1' and cname != '*' order by phid_cid asc, step asc, orderby asc " ; //2020-03-18 : 3Depth 처리
	$result = mysqliQuery($sql, $dbconn) ;
	while( $row = mysql_fetch_array($result) ) {
		$int_cid = $row[0] ;
		$class_i_cid_arr[] = $row[0] ;
		$cname_i_arr[$int_cid] = $row[1] ;
		$class_i_depth_arr[$int_cid] = $row[2] ; 
	}
	...
	if ( !$result ) { 
		...
		//$sql = "select cid, cname from tblClass_int where hid_qid=$qname_id  and is_view='1' and is_drop != '1' order by hid_cid asc, cid asc " ;
		$sql = "select cid, cname, depth from tblClass_int where hid_qid=$qname_id  and is_view='1' and is_drop != '1' and cname != '*' order by phid_cid asc, step asc, orderby asc " ; //2020-03-18 : 3Depth 처리
		$result = mysqliQuery($sql, $dbconn) ;
		while( $row = mysql_fetch_array($result) ) {
			$int_cid = $row[0] ;
			$class_i_cid_arr[] = $row[0] ;
...
if ( $dir == "e" ) {
	...
	$sql = "select cid, cname, depth from $e_tb_name where hid_qid=$qname_id and is_view='1' and is_drop != '1' and cname != '*' order by phid_cid asc, step asc, orderby asc " ; //2020-03-18 : 3Depth 처리
	$result = mysqliQuery($sql, $dbconn) ;
	while( $row = mysql_fetch_array($result) ) {
		$ext_cid = $row[0] ;
		$class_e_cid_arr[] = $row[0] ;
		$cname_e_arr[$ext_cid] = $row[1] ;
		$class_e_depth_arr[$ext_cid] = $row[2] ; 
		//echo $ext_cid . "=>" . $row[1] ;
	}
	...
	if ( !$result ) { 
		...
		//$sql = "select cid, cname from tblClass_int where hid_qid=$qname_id  and is_view='1' and is_drop != '1' order by hid_cid asc, cid asc " ;
		$sql = "select cid, cname, depth from tblClass_ext where hid_qid=$qname_id  and is_view='1' and is_drop != '1' and cname != '*' order by phid_cid asc, step asc, orderby asc " ; //2020-03-18 : 3Depth 처리
		$result = mysqliQuery($sql, $dbconn) ;
...
// 2020-07-30 : 정책 재시작 시 update 하면서 tblClass_int_test 테이블 truncate 되면서 그래프 초기화 현상 막기위해 임시 보완 처리.
/*
if ( !$int_cid || !$ext_cid ) { 
	$class_i_cid_arr = "" ; 
	$cname_i_arr = "" ; 
	...
	//$sql = "select cid, cname from tblClass_int where hid_qid=$qname_id  and is_view='1' and is_drop != '1' order by hid_cid asc, cid asc " ;
	$sql = "select cid, cname from tblClass_int where hid_qid=$qname_id  and is_view='1' and is_drop != '1' and cname != '*' order by phid_cid asc, step asc, orderby asc " ; //2020-03-18 : 3Depth 처리
	$result = mysqliQuery($sql, $dbconn) ;
	while( $row = mysqli_fetch_array($result) ) {
		$int_cid = $row[0] ;
		$class_i_cid_arr[] = $row[0] ;
	...
	//$sql = "select cid, cname from tblClass_ext where hid_qid=$qname_id and is_view='1' and is_drop != '1' order by hid_cid asc, cid asc " ;
	$sql = "select cid, cname from tblClass_ext where hid_qid=$qname_id and is_view='1' and is_drop != '1' and cname != '*' order by phid_cid asc, step asc, orderby asc " ; //2020-03-18 : 3Depth 처리
	$result = mysqliQuery($sql, $dbconn) ;
	while( $row = mysqli_fetch_array($result) ) {
		$ext_cid = $row[0] ;
		$class_e_cid_arr[] = $row[0] ;
...
if ( $dir == "i" ) {
	if ( is_array($class_i_cid_arr) ) { 
		$class_cid_arr = implode(",", $class_i_cid_arr) ;
	} else {
		$class_cid_arr = "";
	}
} else if ( $dir == "e" ) {
	if ( is_array($class_e_cid_arr) ) { 
...
$stime = $_GET["stime"];
$length_val = str_pad(strlen($stime.",".$class_cid_arr), 10, "0", STR_PAD_LEFT);
$msg = "TQP|0099|".$length_val."|".$stime.",".$class_cid_arr ;
//echo $msg ;
...
socket_close($sock);
//echo "TOT_REAL_MSG: ".$tot_real_msg;

// >>>>>>> 230502 hjkim - flow simulator 
// 2,Session log file not found
// 7,Command is running..
if(0) {}
else if(strpos($tot_real_msg, "2,Session log file not found") !== false) {
	http_response_code(404);
	exit;	
} 
else if(strpos($tot_real_msg, "7,Command is running") !== false ) {
...
// cid별 Traffic 받아오기
$traffic_data_arr = explode(",", $traffic_data);
...
for ($i = 1; $i < sizeof($traffic_data_arr); $i++) {
	...
	if ( !$port_flag ) {// ALL 선택한 경우
		...
		$json_i_data .= '{"data":[' . $traffic_realtime . ',' . $int_traffic_val[$traffic_cid] . '],"label":"' . $cname_i_arr[$traffic_cid] . '","color":"#' . $col_arr[$i] . '", "tablen":"' . $i_tb_name . '","depth":"' . $class_i_depth_arr[$traffic_cid] . '"},';

		$json_e_data .= '{"data":[' . $traffic_realtime . ',' . $ext_traffic_val[$traffic_cid] . '],"label":"' . $cname_e_arr[$traffic_cid] . '","color":"#' . $col_arr[$i] . '","depth":"' . $class_e_depth_arr[$traffic_cid] . '"},';

		$json_i_data_pps .= '{"data":[' . $traffic_realtime . ',' . $int_packet_val[$traffic_cid] . '],"label":"' . $cname_i_arr[$traffic_cid] . '","color":"#' . $col_arr[$i] . '","depth":"' . $class_i_depth_arr[$traffic_cid] . '"},';

		$json_e_data_pps .= '{"data":[' . $traffic_realtime . ',' . $ext_packet_val[$traffic_cid] . '],"label":"' . $cname_e_arr[$traffic_cid] . '","color":"#' . $col_arr[$i] . '","depth":"' . $class_e_depth_arr[$traffic_cid] . '"},';
		...
		$json_i_data .= '{"data":[' . $traffic_realtime . ',' . $int_traffic_val[$traffic_cid] . '],"label":"' . $cname_i_arr[$traffic_cid] . '","color":"#' . $col_arr[$i] . '","depth":"' . $class_i_depth_arr[$traffic_cid] . '"},';
