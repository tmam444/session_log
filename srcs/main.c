...
/*   Updated: 2023/04/18 15:41:05 by chulee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	find_files(struct save_file *info)
{
	...
	if (dir == NULL)
		log_message(LOG_ERROR, "Directory Open - %s", directory_path);
...
static void	date_parsing(char *start_time, struct save_file *info)
{
	time_t		end_t;

	if (strptime(start_time, "%Y-%m-%d-%H:%M:%S", &info->start_tm) == NULL)
		log_message(LOG_ERROR, "parsing date string - %s", start_time);
...
int	main(int argc, char *argv[])
{
	...
	if (argc != 2)
		log_message(LOG_ERROR, "Usage - %s YYYY-mm-DD-HH:MM:SS[start_time]", argv[0]);
	file_data = setup(argv[1]);
